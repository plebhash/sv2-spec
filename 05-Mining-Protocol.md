...
| coinbase_prefix             | B0_255         | Up to 8 bytes (not including the length byte) which are to be placed at the beginning of the scriptSig field in the coinbase transaction. The sender MUST NOT send a `coinbase_prefix` longer than 8 bytes.                              |
