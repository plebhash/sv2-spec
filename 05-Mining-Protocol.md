...
For extended channels, the `ntime` constraint from `SubmitSharesStandard` also applies, with the `ntime_start` potentially supplied by `NewExtendedMiningJob` (for immediately active jobs) or `SetCustomMiningJob`.
...
| ntime_start | U32       | The nTime field in the block header at which hashing starts, usually the current time when this message was produced. This is NOT the minimum valid nTime value.                                                                                                                                                                                                                                 |
